...
import shizuka.modules.sql.connection_sql as sql
from shizuka import dispatcher, DRAGONS, DEV_USERS
from shizuka.modules.helper_funcs import chat_status
...
def connected(bot: Bot, update: Update, chat, user_id, need_admin=True):
    ...
    if chat.type == chat.PRIVATE and sql.get_connected_chat(user_id):
        ...
        if ((isadmin) or (isallow and ismember) or (user.id in DRAGONS) or
            ...
            send_message(
                update.effective_message,
                "ᴛʜᴇ ɢʀᴏᴜᴘ ᴄʜᴀɴɢᴇᴅ ᴛʜᴇ ᴄᴏɴɴᴇᴄᴛɪᴏɴ ʀɪɢʜᴛs ᴏʀ ʏᴏᴜ ᴀʀᴇ ɴᴏ ʟᴏɴɢᴇʀ ᴀɴ ᴀᴅᴍɪɴ.\ɴɪ'ᴠᴇ ᴅɪsᴄᴏɴɴᴇᴄᴛᴇᴅ ʏᴏᴜ.",
            )
...
def help_connect_chat(update, context):
    ...
    if update.effective_message.chat.type != "private":
        send_message(update.effective_message,
                     "ᴘᴍ ᴍᴇ ᴡɪᴛʜ ᴛʜᴀᴛ ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ɢᴇᴛ ʜᴇʟᴘ.")
        return
    else:
