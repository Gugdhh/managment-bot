...
import telegram
from shizuka import TIGERS, WOLVES, dispatcher
...
from shizuka.modules.helper_funcs.misc import split_message
from shizuka.modules.helper_funcs.string_handling import split_quotes
from shizuka.modules.log_channel import loggable
from shizuka.modules.sql import warns_sql as sql
...
def warn(user: User,
    ...
    if user.id in TIGERS:
        if warner:
            message.reply_text("ᴛɪɢᴇʀs ᴄᴀɴᴛ ʙᴇ ᴡᴀʀɴᴇᴅ.")
        else:
            message.reply_text(
                "ᴛɪɢᴇʀ ᴛʀɪɢɢᴇʀᴇᴅ ᴀɴ ᴀᴜᴛᴏ ᴡᴀʀɴ ғɪʟᴛᴇʀ!\ɴ ɪ ᴄᴀɴ'ᴛ ᴡᴀʀɴ ᴛɪɢᴇʀs ʙᴜᴛ ᴛʜᴇʏ sʜᴏᴜʟᴅ ᴀᴠᴏɪᴅ ᴀʙᴜsɪɴɢ ᴛʜɪs."
            )
        return

    if user.id in WOLVES:
        if warner:
            message.reply_text("ᴡᴏʟғ ᴅɪsᴀsᴛᴇʀs ᴀʀᴇ ᴡᴀʀɴ ɪᴍᴍᴜɴᴇ.")
        else:
            message.reply_text(
                "ᴡᴏʟғ ᴅɪsᴀsᴛᴇʀ ᴛʀɪɢɢᴇʀᴇᴅ ᴀɴ ᴀᴜᴛᴏ ᴡᴀʀɴ ғɪʟᴛᴇʀ!\ɴɪ ᴄᴀɴ'ᴛ ᴡᴀʀɴ ᴡᴏʟᴠᴇs ʙᴜᴛ ᴛʜᴇʏ sʜᴏᴜʟᴅ ᴀᴠᴏɪᴅ ᴀʙᴜsɪɴɢ ᴛʜɪs."
            )
        return
...
def warn_user(update: Update, context: CallbackContext) -> str:
    ...
    if user_id:
        ...
        message.reply_text("ᴛʜᴀᴛ ʟᴏᴏᴋs ʟɪᴋᴇ ᴀɴ ɪɴᴠᴀʟɪᴅ ᴜsᴇʀ ɪᴅ ᴛᴏ ᴍᴇ.")
    return ""
...
def rmwarn_cmd(update: Update, context: CallbackContext) -> str:
    ...
    if user_id:
        ...
        if warns and warns[0] !=0:
            ...
            reply_text = f"ᴛʜɪs ᴜsᴇʀ ʜᴀs {num_warns}/{limit} ᴡᴀʀɴs."
            try:
            ...
            message.reply_text("ᴛʜɪs ᴜsᴇʀ ᴅᴏᴇsɴ'ᴛ ʜᴀᴠᴇ ᴀɴʏ ᴡᴀʀɴs.")
    else:
        message.reply_text("ɴᴏ ᴜsᴇʀ ʜᴀs ʙᴇᴇɴ ᴍᴇɴᴛɪᴏɴᴇᴅ.")
    return ""
...
def warns(update: Update, context: CallbackContext):
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            text = f"ᴛʜɪs ᴜsᴇʀ ʜᴀs {num_warns}/{limit} ᴡᴀʀɴs, ғᴏʀ ᴛʜᴇ ғᴏʟʟᴏᴡɪɴɢ ʀᴇᴀsᴏɴs:"
        ...
        update.effective_message.reply_text("ᴛʜɪs ᴜsᴇʀ ᴅᴏᴇsɴ'ᴛ ʜᴀᴠᴇ ᴀɴʏ ᴡᴀʀɴs!")
...
def remove_warn_filter(update: Update, context: CallbackContext):
    ...
    for filt in chat_filters:
        if filt == to_remove:
            ...
            msg.reply_text("ᴏᴋᴀʏ, ɪ'ʟʟ sᴛᴏᴘ ᴡᴀʀɴɪɴɢ ᴘᴇᴏᴘʟᴇ ғᴏʀ ᴛʜᴀᴛ.")
            raise DispatcherHandlerStop
...
def list_warn_filters(update: Update, context: CallbackContext):
    ...
    if not all_handlers:
        update.effective_message.reply_text(
            "ɴᴏ ᴡᴀʀɴɪɴɢ ғɪʟᴛᴇʀs ᴀʀᴇ ᴀᴄᴛɪᴠᴇ ʜᴇʀᴇ!")
        return
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"<b>ᴀᴅᴍɪɴ:</b> {mention_html(user.id, user.first_name)}\n"
                f"ʜᴀs ᴇɴᴀʙʟᴇᴅ sᴛʀᴏɴɢ ᴡᴀʀɴs. ᴜsᴇʀs ᴡɪʟʟ ʙᴇ sᴇʀɪᴏᴜsʟʏ ᴘᴜɴᴄʜᴇᴅ.(banned)"
            )
            ...
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"<b>ᴀᴅᴍɪɴ:</b> {mention_html(user.id, user.first_name)}\n"
                f"ʜᴀs ᴅɪsᴀʙʟᴇᴅ sᴛʀᴏɴɢ ᴘᴜɴᴄʜᴇs. ɪ ᴡɪʟʟ ᴜsᴇ ɴᴏʀᴍᴀʟ ᴘᴜɴᴄʜ ᴏɴ ᴜsᴇʀs."
            )
        ...
        if soft_warn:
            msg.reply_text(
                "ᴡᴀʀɴs ᴀʀᴇ ᴄᴜʀʀᴇɴᴛʟʏ sᴇᴛ ᴛᴏ *punch* ᴜsᴇʀs ᴡʜᴇɴ ᴛʜᴇʏ ᴇxᴄᴇᴇᴅ ᴛʜᴇ ʟɪᴍɪᴛs.",
                parse_mode=ParseMode.MARKDOWN)
        else:
            msg.reply_text(
                "ᴡᴀʀɴs ᴀʀᴇ ᴄᴜʀʀᴇɴᴛʟʏ sᴇᴛ ᴛᴏ *Ban* ᴜsᴇʀs ᴡʜᴇɴ ᴛʜᴇʏ ᴇxᴄᴇᴇᴅ ᴛʜᴇ ʟɪᴍɪᴛs.",
                parse_mode=ParseMode.MARKDOWN)
    return ""
